...
----------------------------------------------------------*/
; (function (a, b) {
    var wxUpImg = function (cfg) {
        cfg = cfg || {};
        //微信权限验证接口
        this.apiUrl = "http://api.weixin.gyyx.cn/jsapi/getjsconfig.ashx";
        this.cfg = cfg;
        this.init();
    };
    wxUpImg.prototype = {
        ...
        wxReadyImgPage: function () {
            ...
            wx.ready(function () {
                var chooseBtn = document.getElementById(_this.cfg.btn);  //上传图片按钮
                chooseBtn.onclick = function () {
                    var imgLen = document.querySelectorAll(_this.cfg.imgList).length; //用来判断还能上传几张
                    var count = parseInt(_this.cfg.imgNum - imgLen);
                    if (count === 0) {
                        var str = "亲，最多上传" + _this.cfg.imgNum + "张封面哦！删除后再上传封面";
                        alert(str);
                    } else {
                        wx.chooseImage({
                            count: count, // 默认9
                        ...
                        wx.uploadImage({
                            ...
                            success: function (res) {                              
                                ...
                                if (i < length) {
                                    ...
                                    var imgHtmlItem = document.querySelectorAll(_this.cfg.imgList);
                                    var imgStr = "";
                                    ...
                                    if (_this.cfg.imglistDelBtn) {
                                        ...
                                        var list = document.querySelectorAll(_this.cfg.imglistDelBtn);
                                        for (var o = 0, len = list.length ; o < len; o++) {
                                            list[o].addEventListener("click", function () {
                                                var parents = this.parentNode.parentNode;
                                                var childs = this.parentNode;
        ...
        getCfg: function () {
            ...
            scr.src = this.apiUrl + "?weixinType=" + this.cfg.game + "&SourceUrl=" + encodeURI(window.location.href).replace(/&/g, "%26").replace(/=/g, "%3d") + "&isjavascript=true&debug=" + (this.cfg.debug && 1 == this.cfg.debug ? "true" : "false") + "&jsApiList=%27chooseImage%27,%20%27previewImage%27,%20%27uploadImage%27,%20%27downloadImage%27,%20%27onMenuShareTimeline%27";
            document.getElementsByTagName("head")[0].appendChild(scr);
        }
    },
    window.wxUpImg = function (cfg) {
        return new wxUpImg(cfg);
    }
})(window, document);